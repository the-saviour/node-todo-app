...
app.post('/todos',authenticate, (req,res)=>{
	var todo =new Todo({
		text: req.body.text,
		_creator: req.user._id		//associate todo and user database
...
app.get('/todos',authenticate, (req,res)=>{
	Todo.find({
		_creator: req.user.id}).then((todos)=>{
...
app.patch('/todos/:id', authenticate, (req,res)=> {
	...
	Todo.findOneAndUpdate({
		_id : id,
		_creator: req.user._id} , {$set: body}, {new : true}).then((todo) => {		 // new set to true retruns new object
		if(!todo) {
